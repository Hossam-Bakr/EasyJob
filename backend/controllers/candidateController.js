...
exports.getCandidatesByCompanyCategories = catchAsync(async (req, res) => {
  ...
  if (!company || !company.Industry) {
    return next(new ApiError("Company or industry not found", 404));
  }
  ...
  if (!categories.length) {
    return next(new ApiError("No categories found for this industry", 404));
  }
  ...
  if (filter) {
    ...
    if (filter.language) {
      const languageArray = Array.isArray(filter.language)
        ? filter.language
        : [filter.language];
    ...
    if (filter.jobType) {
      const jobTypeArray = Array.isArray(filter.jobType)
        ? filter.jobType
        : [filter.jobType];
      const jobTypeQueries = jobTypeArray.map((type) =>
        sequelize.where(
          sequelize.fn("JSON_CONTAINS", sequelize.col("jobTypes"), `"${type}"`),
          1
        )
      );
    ...
    if (filter.jobCategories) {
      const jobCategoriesArray = Array.isArray(filter.jobCategories)

...
const UpdateSkillForm = ({
    ...
    onSuccess: (data) => {
      ...
      if (data.status === "success") {
        ...
        setResponseMessage({
          ...
          content: "Skill has been updated Successfully",
        });
        ...
        setResponseMessage({
          title: "Request Faild",
          content: "Skill faild to be Updated please try again",
        });
  ...
  const UpdateOnSubmit = (values) => {
    console.log(values);
    let skillName = { name: values.name };
    mutate({
      type: `${values.skillId}`,
      method: "patch",
      formData: skillName,
    ...
    >
      <Form className={styles.emp_form}>
        ...
        <div className="text-end">
            <button type="submit" className={`${styles.emp_btn} ${styles.skill_btn}`}>
            Update Skill
            </button>
        </div>
